...
pub fn evaluate_const_expr(expr: &typed_ast::Expression) -> Result<Exponent> {
    let name =
        match expr {
            typed_ast::Expression::Scalar { span, value, .. } => {
                return Ok(to_rational_exponent(value.to_f64())
            ...
            typed_ast::Expression::UnaryOperator {
                op: ast::UnaryOperator::Factorial(_),
                ..
            } => "factorial",
            typed_ast::Expression::UnaryOperator {
                op: ast::UnaryOperator::LogicalNeg,
                ..
            } => "logical",

            e @ typed_ast::Expression::BinaryOperator {
                op,
                lhs: lhs_expr,
                rhs: rhs_expr,
                ..
                ...
                return match op {
                    ...
                    typed_ast::BinaryOperator::Div => {
                        if rhs == Rational::zero() {
                            ...
                            Ok(lhs.checked_div(&rhs).ok_or_else(|| {
                    ...
                    | typed_ast::BinaryOperator::LogicalOr => Err(Box::new(
            ...
            typed_ast::Expression::FunctionCall { .. } => "function call",
            typed_ast::Expression::CallableCall { .. } => "function call",
            ...
            typed_ast::Expression::Array { .. } => "array literals",
